...
# Additional SANs served by the same site (e.g. the www alias). Space-separated.
# Overridable via env for the curl one-liner:  EXTRA_DOMAINS="www.foo.net a.foo.net"
# Empty (EXTRA_DOMAINS="") = no extra SANs. To edit in-file, change the default
# string below. (No-colon ${VAR-default}: an explicit empty value stays empty.)
read -r -a EXTRA_DOMAINS <<< "${EXTRA_DOMAINS-www.example.com}"
...
# Where your .html pages are staged. Defaults to the sudo-invoking user's home
# (e.g. /home/admin/site) so you can scp pages straight in — root SSH login is
# disabled, so /root isn't reachable by scp. Override via env or edit here.
SITE_SRC="${SITE_SRC:-/home/${SUDO_USER:-admin}/site}"
...
HSTS_INCLUDE_SUBDOMAINS="${HSTS_INCLUDE_SUBDOMAINS:-0}"   # opt-in: forces HTTPS on ALL subdomains (sticky)
...
for d in "${EXTRA_DOMAINS[@]}"; do
  if [[ "$d" == "example.com" || "$d" == *.example.com ]]; then
    echo "[-] EXTRA_DOMAINS still contains the placeholder '${d}' — set it (or empty it)." >&2
    echo "    Via the curl one-liner: EXTRA_DOMAINS=\"www.yourdomain\" (or EXTRA_DOMAINS=\"\")." >&2
    exit 1
  fi
...
if [[ ! -d "$SITE_SRC" ]]; then
  # First-run bootstrap: create the staging dir and stop cleanly. This runs in
  # preflight, before any install/cert side effects, so nothing else is touched.
  # Own it by the invoking (sudo) user — not root — so you can scp pages in.
  stage_owner="${SUDO_USER:-root}"
  stage_group="$(id -gn "$stage_owner" 2>/dev/null || echo "$stage_owner")"
  echo "[*] SITE_SRC '${SITE_SRC}' does not exist — creating it (${stage_owner}:${stage_group}, 0755)."
  mkdir -p "$SITE_SRC"
  chown "${stage_owner}:${stage_group}" "$SITE_SRC"
  chmod 0755 "$SITE_SRC"
  echo "[+] Created ${SITE_SRC}. Stage your .html pages there (landing page named"
  echo "    '${INDEX_FILE}') — e.g.  scp -r ./site/* ${stage_owner}@<host>:${SITE_SRC}/"
  echo "    — then re-run this script. Nothing else changed."
  exit 0
fi
if ! find "$SITE_SRC" -maxdepth 2 -type f -name '*.html' -print -quit | grep -q .; then
  echo "[-] No .html files found under SITE_SRC '${SITE_SRC}'. Stage your pages and re-run." >&2
  exit 1
fi
...
# ── nginx hardening: server-context snippet + http-context conf.d ────────────
# Per-request tuning goes in a SERVER-context snippet (included in the vhost),
# NOT in conf.d. Debian's nginx.conf already sets server_tokens and
# keepalive_timeout in the http context; repeating them in conf.d (also http
# context) is a fatal "duplicate directive" error. At server level they cleanly
# override the http defaults. conf.d holds only http-only directives that Debian
# does not already set (the limit_req zone, and the optional anon map).

echo "[*] Writing ${HARDENING_SNIPPET}"
cat >"$HARDENING_SNIPPET" <<'EOF'
# Managed by phase3-webserver.sh — included in each server block.
...
if [[ "$ANONYMIZE_LOG_IPS" -eq 1 ]]; then
  # access_log at server level OVERRIDES nginx.conf's http-level access_log for
  # this vhost (a second access_log in http context would log twice, not replace).
  echo 'access_log /var/log/nginx/access.log anon;' >>"$HARDENING_SNIPPET"
fi

echo "[*] Writing ${HARDENING_CONF}"
cat >"$HARDENING_CONF" <<'EOF'
# Managed by phase3-webserver.sh — do not hand-edit; re-run the script.
# http-context-only directives that do NOT clash with Debian's nginx.conf.
EOF

if [[ "$ANONYMIZE_LOG_IPS" -eq 1 ]]; then
  cat >>"$HARDENING_CONF" <<'EOF'

# Privacy: mask the last IPv4 octet / last IPv6 group before logging. The
# map + log_format are http-only; the access_log that USES this format lives in
# the server-context hardening snippet so it overrides the default access_log.
...
if [[ "$ENABLE_NGINX_RATELIMIT" -eq 1 ]]; then
  # limit_req zone (http-only). Keyed on client IP. Violations are logged to
...
# Phase 2 is the preferred owner of fail2ban daemon policy. Phase 3 owns the
# nginx-limit-req jail file, and may install + start fail2ban as a fallback when
# the nginx jail is enabled and the daemon isn't already present. The jail reads
...
# Declarative cleanup: if the jail is no longer wanted (either toggle off), drop
# any jail file a previous run wrote, so it doesn't survive a fail2ban reload.
if [[ -f "$F2B_NGINX_JAIL" && ! ( "$ENABLE_NGINX_FAIL2BAN_JAIL" -eq 1 && "$ENABLE_NGINX_RATELIMIT" -eq 1 ) ]]; then
  echo "[*] Removing stale fail2ban nginx-limit-req jail → ${F2B_NGINX_JAIL}"
  rm -f "$F2B_NGINX_JAIL"
  if command -v fail2ban-client >/dev/null 2>&1 && systemctl is-active --quiet fail2ban; then
    systemctl reload fail2ban 2>/dev/null || systemctl restart fail2ban
  fi
  echo "[+] fail2ban nginx-limit-req jail removed."
fi
...
- Global hardening       : ${HARDENING_SNIPPET} (server) + ${HARDENING_CONF} (http)
...
if [[ "$WRITE_MOTD" -eq 1 ]]; then
  ...
  motd_owner="${SUDO_USER:-admin}"
  cat >"$MOTD_DROPIN" <<EOF
#!/bin/sh
# Managed by phase3-webserver.sh — do not hand-edit; re-run the script.
printf '\n  Web server: ${PRIMARY_DOMAIN}\n'
printf '  ────────────────────────────────────────────────────────\n'
printf '  Pages staged in: ${SITE_SRC}\n'
printf '                   (owned by ${motd_owner} — scp-writable; /root is not)\n'
printf '\n'
printf '  Upload from your machine:\n'
printf '    scp -r ./site/* ${motd_owner}@<host>:${SITE_SRC}/\n'
printf '  Then re-run on this host to publish:\n'
printf '    sudo /usr/local/sbin/phase3-webserver.sh\n'
printf '  (rsyncs into the docroot: ${WEBROOT})\n'
printf '  ────────────────────────────────────────────────────────\n\n'
EOF
