...
#
#  Files this script owns (each carries a "Managed/Generated by phase3-webserver.sh" header):
#      /etc/nginx/conf.d/00-hardening.conf                  (http-context tuning)
#      /etc/nginx/snippets/hardening.conf                   (server-context tuning)
...
#      /etc/fail2ban/jail.d/nginx.local                     (when the nginx jail is on)
#      /etc/update-motd.d/30-webserver                      (when WRITE_MOTD=1)
#      /root/WEBSERVER-RUNBOOK.md                            (when WRITE_RUNBOOK=1)
# ─────────────────────────────────────────────────────────────────────────────
...
curl_tls_opts=(-fsS)
[[ "$STAGING" -eq 1 ]] && curl_tls_opts+=(-k)   # staging cert isn't browser-trusted; don't verify locally
code_https="$(curl "${curl_tls_opts[@]}" -o /dev/null -w '%{http_code}' \
...
EOF
  # Staging-cert recovery — only while a staging cert is in use. Disappears once
  # you re-run with STAGING=0 (the MOTD is rewritten every run).
  if [[ "$STAGING" -eq 1 ]]; then
    cat >>"$MOTD_DROPIN" <<EOF
printf '\n'
printf '  !! STAGING certificate — browsers will NOT trust it.\n'
printf '  Switch to a trusted (production) cert:\n'
printf '    sudo certbot delete --cert-name ${PRIMARY_DOMAIN}\n'
printf '    sudo /usr/local/sbin/phase3-webserver.sh\n'
EOF
  fi
  # Closing separator (literal — quoted heredoc, no expansion).
  cat >>"$MOTD_DROPIN" <<'EOF'
printf '  ────────────────────────────────────────────────────────\n\n'
EOF
...
echo "─────────────────────────────────────────────────────────────────────────"
if [[ "$STAGING" -eq 1 ]]; then
  echo "  [!] STAGING cert issued — browsers will NOT trust it."
  echo "      Switch to production (re-running alone won't replace an existing cert):"
  echo "        sudo certbot delete --cert-name ${PRIMARY_DOMAIN}"
  echo "        sudo /usr/local/sbin/phase3-webserver.sh"
fi
echo
